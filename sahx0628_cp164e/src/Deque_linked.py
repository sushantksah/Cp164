...
class Deque:
    ...
    def is_mirror(self):
        ...
        if self._front is None or self._front == self._rear:
            ...
            idx1 = self._front
            idx2 = self._front
        while idx2 is not None and idx2._next is not None:
            idx1 = idx1._next
            idx2 = idx2._next._next
        reversed_deque = Deque()
        current = idx1

        while current is not None:
            reversed_deque.insert_front(current._value)
            current = current._next

        current = self._front
        reversed_current = reversed_deque._front
        while current is not None and reversed_current is not None:
            if current._value != reversed_current._value:
                mirror = False

            current = current._next
            reversed_current = reversed_current._next
        else:
            mirror = True
